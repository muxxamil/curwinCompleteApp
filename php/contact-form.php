...
$debug = 0;
$subject = ( isset($_POST['subject']) ) ? $_POST['subject'] : 'NewsLetter Subscription';
$emailFieldKey = 'email';
if($subject == 'Email Subscription') {
	$emailFieldKey = 'newsletterEmail';
}
...
foreach($_POST as $label => $value) {
	$label = ($label == $emailFieldKey) ? 'Email' : ucwords($label);
...
try {
	...
	$mail->SetFrom($email, !empty($_POST['name']) ? $_POST['name'] : $_POST[$emailFieldKey]);
	$mail->AddReplyTo($_POST[$emailFieldKey], !empty($_POST['name']) ? $_POST['name'] : $_POST[$emailFieldKey]);
